...
const INDEX_NAME = 'game-sections';
const GAME_SECTIONS_CONTENT_TYPES = ['igCarouselA', 'igCarouselB', 'igGridASection','igGridBSection','igGridCSection','igGridDSection', 'igGridESection', 'igGridFSection', 'igGridGSection', 'igJackpotsSection', 'igDfgSection', 'igJackpotSectionsBlock', 'igSearchResults', 'igGameShuffle'];

function useGameSectionsIndex() {
    ...
    const [osGameSectionsContent, setOsGameSectionsContent] = useState([]);

    const getOpensearchGameSections = async () => {
        setQueryProgress(0.1);
        try {
            const { data, status } = await getAllDocuments(axiosClient, INDEX_NAME, setQueryProgress);

            if (status === 200) {
                setOsGameSectionsContent(data);
    ...
    const syncAllGameSectionTypes = useCallback(async () => {
    ...
    useEffect(() => {
        setOsGameSectionsContent([]);
    }, [selectedEnv]);

    return {
        osGameSectionsContent,
        getOpensearchGameSections,
        syncIgSections: syncAllGameSectionTypes
    };
}

export default useGameSectionsIndex;
