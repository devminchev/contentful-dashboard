...
const INDEX_NAME = 'ml-personalised-sections-defaults';
const CONTENT_TYPE = 'igSuggestedGames';

function useMlDefaultGamesIndex() {
    ...
    const [osMlDefaultGamesContent, setOsMlDefaultGamesContent] = useState([]);

    const getOpensearchMlDefaultGames = async () => {
        setQueryProgress(0.1);
        try {
            const { data, status } = await getAllDocuments(axiosClient, INDEX_NAME, setQueryProgress);

            if (status === 200) {
                setOsMlDefaultGamesContent(data);
    ...
    const syncMlDefaultGamesType = useCallback(async () => {
    ...
    useEffect(() => {
        setOsMlDefaultGamesContent([]);
    }, [selectedEnv]);

    return {
        osMlDefaultGamesContent,
        getOpensearchMlDefaultGames,
        syncMlDefaultGamesType
    };
}

export default useMlDefaultGamesIndex;
