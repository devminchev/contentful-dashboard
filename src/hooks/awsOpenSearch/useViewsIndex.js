...
const INDEX_NAME = 'views';
const GAME_SECTIONS_CONTENT_TYPES = ['igView', 'igMiniGames'];

function useViewsIndex() {
    ...
    const [osViewsContent, setOsViewsContent] = useState([]);

    const getOpensearchViews = async () => {
        setQueryProgress(0.1);
        try {
            const { data, status } = await getAllDocuments(axiosClient, INDEX_NAME, setQueryProgress);

            if (status === 200) {
                setOsViewsContent(data);
    ...
    const syncAllViewsTypes = useCallback(async () => {
    ...
    return {
        osViewsContent,
        getOpensearchViews,
        syncViews: syncAllViewsTypes
    };
}

export default useViewsIndex;
