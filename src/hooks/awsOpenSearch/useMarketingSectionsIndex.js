...
const INDEX_NAME = 'marketing-sections';
const GAME_SECTIONS_CONTENT_TYPES = ['igMarketingSection', 'igBanner', 'igBrazePromosSection', 'igPromotionsGrid'];

function useMarketingSectionsIndex() {
    ...
    const [osMarketingSectionsContent, setOsMarketingSectionsContent] = useState([]);

    const getOpensearchMarketingSections = async () => {
        setQueryProgress(0.1);
        try {
            const { data, status } = await getAllDocuments(axiosClient, INDEX_NAME, setQueryProgress);

            if (status === 200) {
                setOsMarketingSectionsContent(data);
    ...
    const syncAllMarketingTypes = useCallback(async () => {
    ...
    useEffect(() => {
        setOsMarketingSectionsContent([]);
    }, [selectedEnv]);

    return {
        osMarketingSectionsContent,
        getOpensearchMarketingSections,
        syncMarketingSections: syncAllMarketingTypes
    };
}

export default useMarketingSectionsIndex;
