...
const INDEX_NAME = 'navigation';
const GAME_SECTIONS_CONTENT_TYPES = ['igLink', 'igQuickLinks', 'igNavigation'];

function useNavigationIndex() {
    ...
    const [osNavigationContent, setOsNavigationContent] = useState([]);

    const getOpensearchNavigation = async () => {
        setQueryProgress(0.1);
        try {
            const { data, status } = await getAllDocuments(axiosClient, INDEX_NAME, setQueryProgress);

            if (status === 200) {
                setOsNavigationContent(data);
    ...
    const syncAllNavigationTypes = useCallback(async () => {
    ...
    useEffect(() => {
        setOsNavigationContent([]);
    }, [selectedEnv]);

    return {
        osNavigationContent,
        getOpensearchNavigation,
        syncNavigation: syncAllNavigationTypes
    };
}

export default useNavigationIndex;
